-- CRETMDX — ONE-STEP Supabase setup + data load.
-- Paste this ENTIRE file into Supabase -> SQL Editor -> New query -> Run.
-- Plain SQL only, correct order, safe to re-run (it clears and reloads the data).

-- 1) Tables ------------------------------------------------------------------
create table if not exists public.properties (
  id            bigint generated always as identity primary key,
  name text not null, address text, city text, state text, zip text,
  country text default 'USA', lat double precision, lng double precision,
  property_type text default 'NOP Hub', rentable_sqft integer default 0,
  status text default 'Active', ownership text default 'Leased', notes text,
  created_at timestamptz default now()
);
create table if not exists public.leases (
  id bigint generated always as identity primary key,
  property_id bigint references public.properties(id) on delete cascade,
  lease_name text not null, counterparty text, lease_type text default 'Direct',
  role text default 'Tenant', commencement_date text, expiration_date text,
  rentable_sqft integer default 0, base_rent_annual numeric default 0,
  escalation_pct numeric default 0, opex_psf numeric default 0,
  free_rent_months integer default 0, ti_allowance_psf numeric default 0,
  security_deposit numeric default 0, renewal_options text,
  notice_period_months integer default 6, status text default 'Active',
  execution_date text, rent_start_date text, duration_months integer default 0,
  usable_sqft integer default 0, loss_factor numeric default 0,
  building_type text, property_use text, lead_broker text, rent_calc_type text,
  currency text default 'USD', parking_spaces integer default 0,
  parking_rate_monthly numeric default 0, notes text,
  created_at timestamptz default now()
);
create table if not exists public.transactions (
  id bigint generated always as identity primary key,
  name text not null, property_id bigint references public.properties(id) on delete set null,
  type text default 'New Lease', stage text default 'Prospecting', market text,
  target_sqft integer default 0, estimated_value numeric default 0,
  probability integer default 50, broker text, lead text, start_date text,
  target_close_date text, notes text, created_at timestamptz default now()
);

-- 2) Security: only signed-in users can read/write ---------------------------
alter table public.properties   enable row level security;
alter table public.leases       enable row level security;
alter table public.transactions enable row level security;

drop policy if exists "authenticated full access" on public.properties;
create policy "authenticated full access" on public.properties for all to authenticated using (true) with check (true);
drop policy if exists "authenticated full access" on public.leases;
create policy "authenticated full access" on public.leases for all to authenticated using (true) with check (true);
...
-- 3) Load the portfolio (clears first so re-running can't duplicate) ----------
